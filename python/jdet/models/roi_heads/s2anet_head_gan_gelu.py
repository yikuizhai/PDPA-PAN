...
from jdet.ops.orn import ORConv2d, RotationInvariantPooling_LN
...
class S2ANetHead_gan_gelu(nn.Module):
    ...
    def __init__(self,
        ...
        super(S2ANetHead_gan_gelu, self).__init__()
        ...
        # change start
        gan_cfg = train_cfg["gan_cfg"]
        self.loss_type = gan_cfg["loss_type"]
        self.loss_weight = gan_cfg["loss_weights"]
        # change stop
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            fam_reg_conv = nn.Sequential(
                nn.Conv2d(chn, self.feat_channels, 3, stride=1, padding=1),
                nn.GELU(),
            )
            self.fam_reg_convs.append(fam_reg_conv)
            fam_cls_conv = nn.Sequential(
                nn.Conv2d(chn, self.feat_channels, 3, stride=1, padding=1),
                nn.GELU(),
            )
            self.fam_cls_convs.append(fam_cls_conv)
        ...
        self.or_pool = RotationInvariantPooling_LN(256, 8)
        ...
        for i in range(self.stacked_convs):
            ...
            odm_reg_conv = nn.Sequential(
                nn.Conv2d(self.feat_channels, self.feat_channels, 3, stride=1, padding=1),
                nn.GELU(),
            )
            odm_cls_conv = nn.Sequential(
                nn.Conv2d(chn, self.feat_channels, 3, stride=1, padding=1),
                nn.GELU(),
            )
            self.odm_reg_convs.append(odm_reg_conv)
            self.odm_cls_convs.append(odm_cls_conv)
    ...
    def forward_single(self, x, stride):
        ...
        return fam_cls_score, fam_bbox_pred, refine_anchor, odm_cls_score, odm_bbox_pred, fam_feat, odm_feat
    ...
    def loss(self,
        ...
        losses_gan = []
        for i in range(len(fam_feats)):
            if self.loss_type == "mse":
                losses_gan.append(nn.mse_loss(odm_feats[i].detach(), fam_feats[i], reduction="mean") * self.loss_weight)
            if self.loss_type == "mae":
                losses_gan.append(nn.l1_loss(odm_feats[i].detach(), fam_feats[i]) * self.loss_weight)
            if self.loss_type == "huber":
                losses_gan.append(nn.smooth_l1_loss(odm_feats[i].detach(), fam_feats[i]) * self.loss_weight)
            if self.loss_type == "kl":
                losses_gan.append(nn.kl_loss(odm_feats[i].detach(), fam_feats[i]) * self.loss_weight)
            if self.loss_type == "js":
                losses_gan.append(nn.js_loss(odm_feats[i].detach(), fam_feats[i]) * self.loss_weight)
    ...
    def execute(self, feats, targets):
        ...
        if self.is_training():
            ...
            return self.get_bboxes(*outs[:-2], self.parse_targets(targets, is_train=False))
