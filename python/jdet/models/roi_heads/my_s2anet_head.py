...
from jdet.ops.orn import ORConv2d, RotationInvariantPooling, RotationInvariantPooling_gn
...
from .SKConv import sSKConv, cSKConv, scSKConv_v1, scSKConv_v2, scSKConv_v3, scSKConv_v4, cSKConv1


@HEADS.register_module()
class MYS2ANetHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 train_cfg=dict(
                     ...
                     odm_cfg=dict(
                         ...
                         pos_weight=-1,
                         debug=False))):
        super(MYS2ANetHead, self).__init__()
        ...
        if self.conv_type != "normal":
            self.stacked_convs = self.stacked_convs // 2
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            if self.conv_type == "cSKConv":
                self.fam_reg_convs.append(cSKConv(chn, self.feat_channels))
                self.fam_cls_convs.append(cSKConv(chn, self.feat_channels))
            if self.conv_type == "cSKConv1":
                self.fam_reg_convs.append(cSKConv1(chn, self.feat_channels))
                self.fam_cls_convs.append(cSKConv1(chn, self.feat_channels))
        ...
        # self.or_pool = RotationInvariantPooling_gn(256, 8)
        ...
        for i in range(self.stacked_convs):
            ...
            if self.conv_type == "cSKConv":
                self.odm_reg_convs.append(cSKConv(self.feat_channels, self.feat_channels))
                self.odm_cls_convs.append(cSKConv(chn, self.feat_channels))
            if self.conv_type == "cSKConv1":
                self.odm_reg_convs.append(cSKConv1(self.feat_channels, self.feat_channels))
                self.odm_cls_convs.append(cSKConv1(chn, self.feat_channels))
    ...
    def init_weights(self):
        if self.conv_type == "normal":
            for i, m in enumerate(self.fam_reg_convs):
                    normal_init(m[0], std=0.01)
            for i, m in enumerate(self.fam_cls_convs):
                    normal_init(m[0], std=0.01)
        ...
        if self.conv_type == "normal":
            for i, m in enumerate(self.odm_reg_convs):
                    normal_init(m[0], std=0.01)
            for i, m in enumerate(self.odm_cls_convs):
                    normal_init(m[0], std=0.01)
