...
from jdet.models.boxes.box_ops import rotated_box_to_bbox_np, rotated_box_to_poly_np

from jdet.utils.registry import DATASETS
from jdet.config import COCO_CLASSES, DOTA1_CLASSES
...
class COCODataset(Dataset):
    ...
    def _read_ann_info(self, img_id):
        ...
        for i, ann in enumerate(ann_info):
            ...
            hboxes, polys = rotated_box_to_bbox_np(np.array(ann["segmentation"]))
            hboxes_ignore, polys_ignore = rotated_box_to_bbox_np(np.array(ann["segmentation"]))
        ...
        ann = dict(
            rboxes=np.array(ann['segmentation']).astype(np.float32),
            hboxes=hboxes.astype(np.float32),
            polys =polys.astype(np.float32),
            #labels=ann['labels'].astype(np.int32),
            labels=np.array(gt_labels).astype(np.int32),
            rboxes_ignore=np.array(ann['segmentation']).astype(np.float32),
