...
class slip_mixup():
    ...
    def mixup(self, img, label):
        ...
        gapw = int(img.size[0] // 2)
        gaph = int(img.size[1] // 2)
        maps = []
        new_map = []
        rans = []
        maps.append(img.crop((0, 0, gapw, gaph)))
        maps.append(img.crop((gapw, 0, img.size[0], gaph)))
        maps.append(img.crop((0, gaph, gapw, img.size[1])))
        maps.append(img.crop((gapw, gaph, img.size[0], img.size[1])))

        for i in range(4):
            temp_ran = random.randint(1, 3)
            if i == 0:
                temp_ran += 1
                rans.append(temp_ran - 1)
            elif i == 1:
                if temp_ran >= 2:
                    temp_ran += 1
                rans.append(temp_ran - 1)
        ...
        label4 = []
        # label_index获取
        all_rboxes = label["rboxes"]
        all_hboxes = label["hboxes"]
        all_polys = label["polys"]
        all_labels = label["labels"]

        for i in range(len(all_polys)):
            min_x = min(all_polys[i][0], all_polys[i][2], all_polys[i][4], all_polys[i][6])
            min_y = min(all_polys[i][1], all_polys[i][3], all_polys[i][5], all_polys[i][7])
            max_x = max(all_polys[i][0], all_polys[i][2], all_polys[i][4], all_polys[i][6])
            max_y = max(all_polys[i][1], all_polys[i][3], all_polys[i][5], all_polys[i][7])
            if min_x >= 0 and min_y >= 0 and max_x <= gapw and max_y <= gaph:
                label1.append(i)
            if min_x >= gapw and min_y >= 0 and max_x <= img.size[0] and max_y <= gaph:
                label2.append(i)
            if min_x >= 0 and min_y >= gaph and max_x <= gapw and max_y <= img.size[1]:
                label3.append(i)
            if min_x >= gapw and min_y >= gaph and max_x <= img.size[0] and max_y <= img.size[1]:
                label4.append(i)

        temp_loc = [(0,0), (gapw, 0), (0, gaph), (gapw, gaph)]
        for i in range(4):
            offset2x, offset2y = temp_loc[i]
            # 转换成numpy
            t1_map = np.array(maps[i])
            t2_map = np.array(maps[rans[i]])
            # 图片mixup
            temp_map = np.round(t1_map * self.prob) + np.round(t2_map * (1 - self.prob))
            ...
            if rans[i] == 0:
                if len(label1) != 0:
                    offset1x, offset1y = temp_loc[0]
                    for temp_index in label1:
                        temp1 = all_rboxes[temp_index]
                        temp1[0] = temp1[0] - offset1x + offset2x
                        temp1[1] = temp1[1] - offset1y + offset2y
                        temp2 = all_hboxes[temp_index]
                        temp2[0] = temp2[0] - offset1x + offset2x
                        temp2[1] = temp2[1] - offset1y + offset2y
                        temp2[2] = temp2[2] - offset1x + offset2x
                        temp2[3] = temp2[3] - offset1y + offset2y
                        temp3 = all_polys[temp_index]
                        temp3[0] = temp3[0] - offset1x + offset2x
                        temp3[1] = temp3[1] - offset1y + offset2y
                        temp3[2] = temp3[2] - offset1x + offset2x
                        temp3[3] = temp3[3] - offset1y + offset2y
                        temp3[4] = temp3[4] - offset1x + offset2x
                        temp3[5] = temp3[5] - offset1y + offset2y
                        temp3[6] = temp3[6] - offset1x + offset2x
                        temp3[7] = temp3[7] - offset1y + offset2y
                        temp4 = all_labels[temp_index]
                        all_rboxes = np.concatenate((all_rboxes, [temp1]), axis=0)
                        all_hboxes = np.concatenate((all_hboxes, [temp2]), axis=0)
                        all_polys = np.concatenate((all_polys, [temp3]), axis=0)
                        all_labels = np.concatenate((all_labels, [temp4]), axis=0)
            elif rans[i] == 1:
                if len(label2) != 0:
                    offset1x, offset1y = temp_loc[1]
                    for temp_index in label2:
                        temp1 = all_rboxes[temp_index]
                        temp1[0] = temp1[0] - offset1x + offset2x
                        temp1[1] = temp1[1] - offset1y + offset2y
                        temp2 = all_hboxes[temp_index]
                ...
                if len(label3) != 0:
                    offset1x, offset1y = temp_loc[2]
                    for temp_index in label3:
                        temp1 = all_rboxes[temp_index]
                        temp1[0] = temp1[0] - offset1x + offset2x
                        temp1[1] = temp1[1] - offset1y + offset2y
                        temp2 = all_hboxes[temp_index]
                ...
                if len(label4) != 0:
                    offset1x, offset1y = temp_loc[3]
                    for temp_index in label4:
                        temp1 = all_rboxes[temp_index]
                        temp1[0] = temp1[0] - offset1x + offset2x
                        temp1[1] = temp1[1] - offset1y + offset2y
                        temp2 = all_hboxes[temp_index]
                        ...
                        all_polys = np.concatenate((all_polys, [temp3]), axis=0)
                        all_labels = np.concatenate((all_labels, [temp4]), axis=0)
        label["rboxes"] = all_rboxes
        label["hboxs"] = all_hboxes
        label["polys"] = all_polys
        label["labels"] = all_labels
        # 图片重新拼接
        loc1 = (0, 0)
        loc2 = (gapw, 0)
        loc3 = (0, gaph)
        ...
        new_image.paste(new_map[0], loc1)
        new_image.paste(new_map[1], loc2)
        new_image.paste(new_map[2], loc3)
        new_image.paste(new_map[3], loc4)

        return new_image, label
    def __call__(self, image, target=None):
        return self.mixup(image, target)
